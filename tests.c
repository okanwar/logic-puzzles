...
int test_swapTwoBytes(int x, int n, int m)
{
    /* little endiamachine */
...
int test_hasAnEvenBitSet(int x) {
  int i;
  for (i = 0; i < 32; i+=2)
      if (x & (1<<i))
   return 1;
...
int test_shiftLogically(int x, int n) {
  unsigned u = (unsigned) x;
  unsigned shifted = u >> n;
  return (int) shifted;
}
// two's comp puzzles
int test_isShortSized(int x)
{
  short int sx = (short int) x;
  return x == sx;
}
int test_isMaxTC(int x) {
    return x == 0x7FFFFFFF;
}
int test_subtractOne(void) {
  return -1;
}
int test_fitsInNBits(int x, int n)
{
  int TMin_n = -(1 << (n-1));
  int TMax_n = (1 << (n-1)) - 1;
...
int test_getSignBit(int x) {
    if ( !x ) return 0;
    return (x < 0) ? -1 : 1;
}
int test_isLessThan(int x, int y)
{
  return x < y;
}
